...
// .wrangler/tmp/bundle-MBpPSV/checked-fetch.js
...
// .wrangler/tmp/bundle-MBpPSV/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-MBpPSV/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
transactionRouter.get("/get", async (c) => {
  ...
  const { startDate, endDate, category, type, search, sortBy, sortOrder } = c.req.query;
  let categoryId;
  if (category) {
  ...
  if (sortBy) {
    if (sortBy === "category") {
      orderByClause.push({ category: { name: sortOrder === "desc" ? "desc" : "asc" } });
    } else if (sortBy === "date") {
      orderByClause.push({ date: sortOrder === "desc" ? "desc" : "asc" });
    } else if (sortBy === "type") {
      orderByClause.push({ type: sortOrder === "desc" ? "desc" : "asc" });
    }
  }
  const transactions = await prisma.transaction.findMany({
    where: {
      ...
      // Add categoryId filter if category is specified
    },
    orderBy: orderByClause.length ? orderByClause : void 0,
    include: {
      category: true
      // Include the category to allow sorting by category name
    }
  });
  return c.json({ transactions });
});
...
transactionRouter.get("/summary", async (c) => {
  const prisma = new import_edge2.PrismaClient({
    datasourceUrl: c.env.DATABASE_URL
  }).$extends(withAccelerate());
...
transactionRouter.get("/chart-data", async (c) => {
  const prisma = new import_edge2.PrismaClient({
    datasourceUrl: c.env.DATABASE_URL
  }).$extends(withAccelerate());
  const transactions = await prisma.transaction.findMany({
